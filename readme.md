# jdbcBalancer — High-Performance PostgreSQL In-Process Proxy & Load Balancer

Легковесный, высокопроизводительный внутрипроцессный прокси-сервер и
балансировщик нагрузки для **PostgreSQL**. Разработан для прозрачной
аутентификации legacy/JDBC клиентов с конвертацией Cleartext ↔ SCRAM-SHA-256,
поддержки отказоустойчивости (Failover) и предотвращения аномалий пулов
соединений через **IP-Hash Sticky Sessions**.
...
двунаправленного копирования (io.CopyBuffer) с использованием пула буферов sync.Pool.
